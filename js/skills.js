var skill_data = {
	1: {
		...
		desc() { return "The 2nd Energy Upgrade's cost scales "+format(tmp.skills[1].eff.times(100))+"% slower" },
		eff() { return Decimal.sub(1, Decimal.div(1, tmp.skills[1].lvl.plus(1).sqrt())) },
	},
	2: {
		symbol: "&#10683;",
		...
		eff() { return Decimal.sub(1, Decimal.div(1, tmp.skills[2].lvl.plus(1).sqrt())) },
	},
	3: {
		symbol: "&#937;",
		basedOn: "Mega-Energy",
		...
		eff() { return tmp.skills[3].lvl.div(5).sqrt() },
	}
}
...
function getXPGain(x) {
	return skill_data[x].base().times(getXPGainMult(x)).sub(player.mega.xp[x]||0).floor().max(0)
}

function nextXP(x) {
	let end = new Decimal(player.mega.xp[x]||0).plus(tmp.skills[x].xpGain).plus(1).div(getXPGainMult(x))
